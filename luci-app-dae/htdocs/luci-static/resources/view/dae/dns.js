...
return view.extend({
	render() {
		...
		m = new form.Map('dae', _('DNS Settings'),
			_('Configure DNS settings for DAE.'));
		...
		o = s.option(form.TextValue, '_dns_config', _('DNS Configuration'));
		o.rows = 25;
		o.monospace = true;
		o.load = function(section_id) {
			return fs.read_direct('/etc/dae/config.d/dns.dae', 'text')
		...
		o.write = function(section_id, value) {
			value = value.replace(/\r\n?/g, '\n');
			return fs.write('/etc/dae/config.d/dns.dae', value, 384 /* 0600 */)
			.catch(function(e) {
				ui.addNotification(null, E('p', e.message));
			});
		};
		...
		return m.render().then(function(node) {
			let textarea = node.querySelector('textarea[name="cbid.dae.config._dns_config"]');
			if (textarea) {
				initEditor(textarea);
			}
			return node;
